<template>
	<div id = 'todo-list' @dataCollected = 'dosmth'>
			<button type="button" id = 'viewButton' class="btn btn-secondary btn-lg" @click = "viewAddMenu" :disabled = 'buttonDisabled'> Добавить заметку</button>

			<div id = 'addDiv' v-if = "viewMenu">
		 				<form id = "createForm">
		 					<input id = "createTitle" class = "form-control" type="textaria" name="title" v-model='title' placeholder="Название заметки" maxlength="32">

			       			<date-picker id = 'datepicker' v-model="date" :config="config"></date-picker>
